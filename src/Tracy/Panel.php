...
class Panel implements Tracy\IBarPanel
{
	...
	public function getTab(): string
	{
		...
		return '<span title="Contributte/Translation">' . $icon . ($this->translator->getLocale() !== null ? ' <strong>' . $this->translator->getLocale() . '</strong>' : '') . ($this->missingTranslationCount > 0 ? ' (' . $this->missingTranslationCount . ' ' . $errMsg . ')' : '') . '</span>';
	}
