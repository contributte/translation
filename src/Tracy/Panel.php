...
class Panel implements Tracy\IBarPanel
{
	...
	public function getTab(): string
	{
		...
		return '<span title="Contributte/Translation">' . $icon . ($this->translator->locale !== null ? ' <strong>' . $this->translator->locale . '</strong>' : '') . ($this->missingTranslationCount > 0 ? ' (' . $this->missingTranslationCount . ' ' . $errMsg . ')' : '') . '</span>';
	}
